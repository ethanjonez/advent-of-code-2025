...
pub fn run() {
    ...
    let wrappings = iterate_parcel_wrapping(&matrix);

    println!("4-2: {}", wrappings);
}

pub fn iterate_parcel_wrapping(matrix: &Vec<Vec<&str>>) -> i64 {
    let mut totalWrappingToRemove: i64 = 0;
    let mut wrappingToAdd: i64 = 0;
    let mut newMatrix: Vec<Vec<&str>> = matrix.clone();

    while true {
        (wrappingToAdd, newMatrix) = find_parcel_wrapping(&newMatrix);

        if wrappingToAdd == 0 {
            break;
        }

        totalWrappingToRemove += wrappingToAdd
    }

    totalWrappingToRemove
}

pub fn find_parcel_wrapping<'a>(matrix: &Vec<Vec<&'a str>>) -> (i64, Vec<Vec<&'a str>>) {
    let mut wrappingCanRmove: i64 = 0;
    let mut newMatrix: Vec<Vec<&str>> = matrix.clone();
